  ...
  <div class="treasury-crud">
    <div class="flex flex-wrap flex-col items-center">
      <div class="w-full lg:w-8/12 my-6">
        ...
        <v-data-table :headers="headers" :items="treasuries" :page.sync="page" :options.sync="options"
            ...
            <template v-slot:item.actions="{ item }">
              <router-link
                  :to="`/admin/treasuries/update/${item.id}`"
                  class="
                    inline-block
                    whitespace-nowrap
                    text-white
          ...
          <div class="flex flex-wrap w-full items-center mt-6">
            <div class="w-1/4 pr-4 text-sm whitespace-nowrap">{{ totalItems == 1 ? totalItems + ' Treasury' :
                totalItems + ' Treasuries'
            }}</div>
...
export default {
  ...
  methods: {
    async deleteTreasury(rowData) {
      ...
      if (ok) {
        API.request(`treasury/delete/${rowData.id}`, null, "DELETE")
          .then((res) => {
            this.treasuries = this.treasuries.filter(e => e.id != rowData.id)
    ...
    getData() {
      ...
      return API.request("/treasury/all", params)
        .then((response) => {
